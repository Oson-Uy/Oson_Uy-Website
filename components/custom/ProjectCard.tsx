        ...
        <Card className="group relative overflow-hidden rounded-[2rem] bg-white shadow-sm hover:shadow-2xl hover:shadow-blue-900/5 transition-all duration-500 border border-slate-200 flex flex-col">
            ...
            <CardContent className="p-6 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-3">
                    <h3 className="text-xl font-bold text-[#1E3A8A] leading-tight group-hover:text-[#F97316] transition-colors">{project.name}</h3>
                ...
                <p className="text-xs text-slate-500 mb-6 leading-relaxed flex items-center gap-1.5 font-medium">
                    <MapPin className="h-3 w-3 text-[#1E3A8A]" /> {project.district}, {project.location}
                </p>

                <div className="flex items-center gap-3 mb-8">
                    <div className="w-8 h-8 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center text-[8px] font-black text-slate-400">DEV</div>
                    <div>
                        <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest leading-none mb-1">Developer</p>
                        <p className="text-xs font-bold text-[#1E3A8A]">{project.developer}</p>
                    </div>
                </div>

                <div className="mt-auto flex gap-3">
                    <Link href={`/catalog/${project.id}`} className="flex-1">
                        <Button variant="ghost" className="w-full bg-slate-50 border border-slate-200 text-[#1E3A8A] text-[10px] font-bold uppercase tracking-wider rounded-xl hover:bg-slate-100 h-10">
                            Details
                        </Button>
                    </Link>
