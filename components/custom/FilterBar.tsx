...
export function FilterBar({ translations }: FilterBarProps) {
    ...
    const inputClasses = "h-14 w-full bg-blue-900/40 border border-blue-500/30 rounded-xl pl-10 pr-4 text-sm font-semibold outline-none focus:ring-2 ring-accent/60 focus:bg-blue-800/50 transition-all placeholder:text-white/35 appearance-none text-white";
    const labelClasses = "text-xs font-bold text-white/55 ml-2 mb-1.5 block uppercase tracking-wider";
    const iconClasses = "absolute left-3.5 top-1/2 -translate-y-1/2 text-accent h-4 w-4 pointer-events-none z-10 opacity-90";

    return (
        <div className="bg-primary/95 backdrop-blur-2xl p-6 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.4)] max-w-[1200px] mx-auto w-full border border-white/10 text-white flex flex-col gap-6">
